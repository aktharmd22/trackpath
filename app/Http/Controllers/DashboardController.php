...
class DashboardController extends Controller
{
    public function index(): Response
    {
        ...
        // Minutes per day over the last 14 days, zero-filled for the chart.
        ...
        $minutesByDay = collect(range(13, 0))
            ->map(function (int $daysAgo) use ($logged) {
                $date = now()->subDays($daysAgo)->toDateString();

                return [
                    'date' => $date,
                    'minutes' => (int) ($logged[$date] ?? 0),
                ];
            })
            ->values();
        ...
        // Explicit follow-up reminders come first; stale stages are the fallback heuristic.
